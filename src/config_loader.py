...
def validate_config(config: dict) -> dict:
    ...
    if "ignore" in config:
        if isinstance(config["ignore"], list):
            ...
            print(f"警告: 'ignore' 字段应为列表，当前类型为 {type(config['ignore']).__name__}。")
    ...
    if "icons" in config:
        if isinstance(config["icons"], dict):
            ...
            print(f"警告: 'icons' 字段应为对象，当前类型为 {type(config['icons']).__name__}。")
...
def load_config() -> dict:
    ...
    if config_path.exists():
        try:
            ...
            print(f"错误: config.json 格式错误 (行 {e.lineno}, 列 {e.colno}): {e.msg}")
            print("程序将尝试使用默认配置继续运行。")
        except Exception as e:
            print(f"警告: 读取 config.json 时发生未知错误: {e}")
    else:
        print("提示: 未找到 config.json，将使用内置默认配置。")
